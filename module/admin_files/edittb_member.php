...
?>
    	<form id="frmedit" name="frmedit"  method="post"  enctype="multipart/form-data">
    		...
    		<input type="tel" class="form-control" id="mem_tel" name="mem_tel" value="<?php echo $row_q['mem_tel'] ?>" >
				</div>
                <div class="form-group"><label for="mem_card">เลขบัตรประชาชน:</label><label id="err4" class="err"></label>    	
    		<input type="texr" class="form-control" id="mem_card" name="mem_card" value="<?php echo $row_q['mem_card'] ?>" >
				</div>
		  ...
		  <button type="reset"   class="btn btn-warning">ล้างข้อมูล</button>
				...
				function checkEmpty()
				{
				 ...
				 $("#err5").html('');
				  ...
				  if($("#mem_id").val() == "")
				  {
				    $("#err1").html('กรุณากำหนดค่า รหัสผู้สมัคร ด้วยค่ะ');
				    $("#mem_id").focus();
				    chk= false;
				  }
				  if($("#act_id").val() == "")
				  ...
				  if(chk){ 
				  	    ...
				  	    var url="module/admin_files/updatetb_member.php"
				  		var form = $('#frmedit')[0];
