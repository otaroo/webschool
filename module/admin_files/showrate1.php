...
Date Create : 2019-01-17 20:06:37
...
<div class="w3-row content_box"><?php
	...
	?>
		...
		<tbody><?php 
			$sql2 ="select * from tb_rate where 1  order by act_id aesc";  
			$qess2=$db->query($sql2);	
			while($fd2=$qess2->fetch_assoc()){	
			
		?><tr>
					<td><?php echo $fd2['rate_id'];?></td>
							$sql3 ="select rate_name name from tb_rate where rate_id='".$fd2['rate_name']."'";  
							$qess3=$db->query($sql3);	
					...
					<td><?php echo $dt->DTtoLDthai($fd3['name']);?></td><?php 
							$sql3 ="select mem_name  name from tb_member where mem_id='".$fd2['mem_id']."'";  
							$qess3=$db->query($sql3);	
							$fd3=$qess3->fetch_assoc();
